...
- Keep every mutation plan-first regardless of execution surface. Call the exact request without `confirm`, review its commands, risk, rollback, and verification with the human, then repeat the same request with `confirm: true` and the returned `confirmationToken`. A missing, changed, replayed, or pre-restart token must not execute and returns a fresh plan/token.
...
- For new table schema DDL, prefer `local_ydb_generate_schema` with structured input, review/validate the generated script, then use `local_ydb_apply_schema`; applying requires the exact plan's `confirmationToken` together with `confirm=true`.
