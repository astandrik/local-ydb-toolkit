...
- Keep every mutation plan-first regardless of execution surface. Call the exact request without `confirm` and review its commands, risk, rollback, and verification with the human. Read the one-time token from the plan response's `confirmation.token`, then repeat the same request with `confirm: true` and that value in the `confirmationToken` request argument. A missing, changed, replayed, or pre-restart token must not execute and returns a fresh plan/token.
- Treat storage reduction and version upgrade as mutually exclusive per-profile rebuilds. The accepted execution holds its lease through cleanup; a plan created while it runs becomes stale after release and must be planned again. Storage reduction also binds mutable image tags to exact Docker image IDs, and auth-enabled rebuilds require distinct normalized destinations for config, dynamic token, and root password artifacts.
- Treat configured auth/password files and the selected standalone restore dump as part of the exact plan. If their contents change after review, obtain a fresh plan/token; never log or persist their contents or private fingerprints.
- Treat `confirmationToken` as an ephemeral process capability: do not log it, store it in reusable notes, or treat possession of it as evidence that a human approved the plan. The MCP host/client remains responsible for obtaining approval immediately before execution.
...
- Treat `profile.dynamicNodeCount` as the total declarative tenant-node count, including the primary node. Configured dynamic names must be distinct from the static container, and standalone primary start uses the same shared-network port validation as bootstrap. Tenant bootstrap publishes static gRPC and every configured dynamic gRPC port on loopback through the static container; one-off nodes do not change those bindings. Before any restart or auth-hardening mutation, require the existing static container to match the full profile and exact configured bindings; a mismatch requires destroy/bootstrap and must leave configured and one-off containers untouched. Bootstrap and compatible restart reconcile nodes `1..N`; both recreate configured containers even when they are already running or restarting. `local_ydb_add_dynamic_nodes` creates one-off runtime nodes starting at `N+1` by default, and an explicit `startIndex` must also be greater than `N`. Static IC port `19001` is reserved in the shared network namespace. Readiness requires a stable running exact container plus its IC registration; a matching nodelist port alone is insufficient. Default removal considers only one-off suffixes above `N`; explicit container, node ID, or `startIndex` targeting can remove a configured suffix and create drift that the next bootstrap or restart restores. Rollback uses bootstrap/restart for configured removals and configured-node recreation, and add with matching suffixes/ports for one-off removals. Before a storage or version rebuild, inspect and preserve every one-off node's exact gRPC, monitoring, and IC ports; abort before dump or destroy when a definition is incomplete.
...
- For new table schema DDL, prefer `local_ydb_generate_schema` with structured input, review/validate the generated script, then use `local_ydb_apply_schema`; applying requires `confirm=true` with the plan response's `confirmation.token` supplied in the `confirmationToken` request argument.
...
- Treat `local_ydb_sql action=execute` as high risk: it must complete mandatory `EXPLAIN`, return a token bound to that current plan, and send one `NoTx` execution only when the exact repeat call supplies that token with `confirm=true`. Never retry an execution whose final status is unknown; obtain a fresh plan/token instead.
