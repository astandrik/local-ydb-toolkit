...
- Read `references/evals.md` when changing this skill, MCP server instructions, MCP prompts, or public tool-selection guidance.
...
- Take a tenant dump before destructive or hard-to-rollback live changes that must preserve data: version upgrade, storage-group reduction (storage-pool reduction), destroy/rebuild, single-disk or bind-mount migration, production-like auth/config hardening, or manual storage surgery. For live or production-like auth/config hardening, rehearsing the change on a copied volume first is an accepted alternative to the dump. Use tenant-wide dumps for full rebuilds and path-level dumps for one table or directory probes.
...
- Before restoring from an existing toolkit dump, use `local_ydb_list_dumps` to choose a valid `dumpName`; restore only after the target tenant is bootstrapped and metadata is reachable. For path-level restore, remember that `path` is the destination directory passed to `ydb tools restore -p`.
