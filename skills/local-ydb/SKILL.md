...
- For generated column tables, use `partitionByHash` only with `store: "column"` and primary key columns. Use top-level `store` instead of `with.STORE`; keep secondary and vector indexes on row-oriented tables.
