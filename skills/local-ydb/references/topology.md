...
Tenant bootstrap publishes static gRPC and every configured dynamic gRPC port on loopback through the static container. One-off nodes do not alter those immutable bindings. A static container created for a smaller configured count fails compatibility before dynamic-node mutation and must be deliberately rebuilt.
