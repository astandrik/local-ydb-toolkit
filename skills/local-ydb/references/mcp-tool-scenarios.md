...
- Issue distinct upgrade and storage-reduction tokens for one profile before either confirm; after one is accepted, every other pre-issued rebuild token for that profile must be rejected. While the accepted rebuild is running, issue a new plan and confirm it as a negative control: the confirm is rejected, the plan becomes stale when the first rebuild and cleanup finish, and a fresh post-cleanup plan can be accepted. A different profile remains independently confirmable.
- Auth-enabled composite rebuilds reject exact or normalized aliases among the configured auth config, dynamic token, and root credential destinations. With distinct destinations, they generate all three artifacts in a private workspace, persist those exact bytes to the configured host paths, and continue auth reapply and node recreation only from the private copies. Replacing a canonical artifact after persistence must not change the applied bytes, and cleanup removes the private workspace.
...
1. Bootstrap with `dynamicNodeCount: 1`. Save the static and primary container IDs, bindings, and running/stopped states.
2. Change the same profile to `dynamicNodeCount: 3`. Restart plan-only must place the full static compatibility check before every mutation. Confirmed restart must reject the missing configured bindings without changing saved IDs/states or creating `-2`/`-3`; tenant bootstrap must reject the same shared compatibility contract.
...
- tenant bootstrap and restart additionally require the GraphShard feature flag plus the static gRPC binding and every configured dynamic gRPC binding
- a static container created for a smaller `dynamicNodeCount` fails both bootstrap and restart compatibility before any configured dynamic container is mutated and requires destroy/rebootstrap
