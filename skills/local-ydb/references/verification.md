...
Before restart, require the plan to show a check-only static compatibility command before every lifecycle mutation. The check must cover image reference and ID, network, `/ydb_data` mount, environment including GraphShard, restart policy, disabled healthcheck, and the exact loopback bindings for static gRPC, monitoring, and every configured dynamic gRPC port. For a `dynamicNodeCount` change that makes bindings incompatible, confirmed restart must fail before any container ID or running/stopped state changes; recovery begins only after a successful static preflight. Rebuild immutable bindings with destroy followed by bootstrap.

Require the same check-only static compatibility command before auth hardening copies config or changes any container state. For standalone primary start, require plan-time rejection of a static-container name alias or a collision with static IC port `19001`. Before storage reduction or version upgrade reaches dump or destroy, record each one-off container's exact `--grpc-port`, `--mon-port`, and `--ic-port`; an incomplete inspection must abort with no destructive command.
