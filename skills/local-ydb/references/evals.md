...
- Reading the installed skill is treated as activation evidence: a positive case requires a recognized content-reader command targeting the installed `skills/local-ydb/SKILL.md` file, while a negative control fails on any such read even when the final answer self-reports `should_use_local_ydb_skill: false`. Listing the skill directory alone is not activation evidence.
