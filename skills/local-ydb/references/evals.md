...
- The command tripwire flags `docker`, `ydb`, or `ydbd` in command position: the start of a command or right after a `;`, `|`, `&`, or newline separator, optionally behind `sudo` or an absolute path. Wrapper chains (`bash -c`), `ssh`, command substitution, pipelines into shells, and quoting/obfuscation tricks are out of scope.
