...
- The command tripwire flags `docker`, `ydb`, or `ydbd` in command position: the start of a command or right after an unquoted `;`, `|`, `&`, or newline separator, optionally behind the standard direct-command prefixes — environment assignments (`VAR=value`) and `sudo` (its common options, combined short options, and the `--` separator are consumed) — or an absolute path. Simple single- and double-quoted arguments are preserved. Wrapper chains (`bash -c`), `ssh`, command substitution, pipelines into shells, and other indirection are out of scope.
- Successfully reading the installed skill is treated as activation evidence: a positive case requires a completed content-reader command using the installed `skills/local-ydb/SKILL.md` file as an input operand in the same shell segment, while a negative control fails on any such successful read even when the final answer self-reports `should_use_local_ydb_skill: false`. Listing the skill directory, a failed read, using the path as a search pattern, or mentioning the path in another command segment is not activation evidence.
