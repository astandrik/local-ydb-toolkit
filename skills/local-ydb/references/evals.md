...
- Reading the installed skill is treated as activation evidence: a positive case requires a recognized trace command that reads the installed `skills/local-ydb` path, while a negative control fails on any such command even when the final answer self-reports `should_use_local_ydb_skill: false`.
