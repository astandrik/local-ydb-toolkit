...
export const localYdbInstructions = [
  ...
  "If local_ydb_check_prerequisites reports installable missing packages, review the plan first and then run it with confirm=true to install supported host helpers such as curl or ruby; Docker still requires manual installation and startup.",
  "Use local_ydb_status_report or local_ydb_inventory first to establish the current stack state before mutating anything. Treat inventory ok=false as an unavailable inventory, not as an empty host; mutating inventory consumers fail closed.",
