...
export const localYdbInstructions = [
  ...
  "For new table schema DDL, prefer local_ydb_generate_schema to render structured CREATE TABLE, ALTER TABLE, DROP TABLE, and index specs, then validate with local_ydb_apply_schema action=validate; action=apply validates again and requires confirm=true before executing generated or raw table DDL. Use store instead of with.STORE, use partitionByHash only with store=column and primary key columns, keep secondary/vector indexes on row tables, use global secondary indexes, keep unique indexes synchronous, apply addColumn before a separate addIndex when the index needs the new column, and provide full GLOBAL SYNC vector_kmeans_tree settings. Prefer adding vector indexes after representative data is loaded.",
