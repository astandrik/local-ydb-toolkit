...
const PullImageArgs = MutatingArgs.extend({
  image: z.string().min(1).optional(),
});

const PullStatusArgs = z.object({
  jobId: z.string().min(1),
});

const UpgradeVersionArgs = MutatingArgs.extend({
...
const localYdbToolIndex = [
  ...
  [
    "dynamic nodes",
    ["local_ydb_add_dynamic_nodes", "local_ydb_remove_dynamic_nodes"],
  ],
  [
    "storage",
    [
  ...
  ["backup restore", ["local_ydb_dump_tenant", "local_ydb_restore_tenant"]],
  [
    "auth",
    [
...
export const localYdbTools: Tool[] = [
  tool(
    "local_ydb_inventory",
    "Collect Docker inventory for a local-ydb target profile.",
    profileSchema(),
  ),
  tool(
    "local_ydb_database_status",
    "Read database status via the YDB admin API.",
    profileSchema(),
  ),
  tool(
    "local_ydb_container_logs",
    "Read recent logs from the static or dynamic local-ydb container.",
    logsSchema(),
  ),
  tool(
    "local_ydb_status_report",
    "Aggregate local-ydb inventory, auth, tenant, and node checks.",
    profileSchema(),
  ),
  tool(
    "local_ydb_tenant_check",
  ...
  tool(
    "local_ydb_nodes_check",
    "Check dynamic nodes through viewer/json nodelist.",
    profileSchema(),
  ),
  tool(
    "local_ydb_graphshard_check",
    "Check GraphShard capability and tablet visibility.",
    profileSchema(),
  ),
  tool(
    "local_ydb_auth_check",
    "Check anonymous viewer and CLI auth posture.",
    profileSchema(),
  ),
  tool(
    "local_ydb_storage_placement",
    "Read storage pool and BSC physical placement.",
    profileSchema(),
  ),
  tool(
    "local_ydb_add_storage_groups",
  ...
  tool(
    "local_ydb_storage_leftovers",
    "Find leftover local-ydb volumes, dumps, and PDisk paths.",
    profileSchema(),
  ),
  tool(
    "local_ydb_list_versions",
  ...
  tool(
    "local_ydb_bootstrap",
    "Bootstrap a GraphShard-ready local-ydb topology.",
    mutatingSchema(),
  ),
  tool(
    "local_ydb_check_prerequisites",
  ...
  tool(
    "local_ydb_create_tenant",
    "Create the configured CMS tenant if missing.",
    mutatingSchema(),
  ),
  tool(
    "local_ydb_start_dynamic_node",
    "Start the configured dynamic tenant node.",
    mutatingSchema(),
  ),
  tool(
  ...
  tool(
    "local_ydb_restart_stack",
    "Restart static and dynamic local-ydb containers.",
    mutatingSchema(),
  ),
  tool(
    "local_ydb_upgrade_version",
  ...
  tool(
    ...
    "Dump the configured tenant using a local-ydb helper container.",
    dumpSchema(),
  ),
  tool(
    "local_ydb_restore_tenant",
    "Restore the configured tenant from a named dump.",
    restoreSchema(),
  ),
  tool(
    "local_ydb_prepare_auth_config",
  ...
  tool(
    "local_ydb_apply_auth_hardening",
    "Apply a reviewed YDB config file and restart local-ydb.",
    authHardeningSchema(),
  ),
  tool(
    "local_ydb_set_root_password",
...
const handlers: Record<string, ToolHandler> = {
  ...
  local_ydb_list_versions: async (args, options) => {
    const parsed = ListVersionsArgs.parse(args ?? {});
    return listVersions({ ...parsed, fetchImpl: options.fetchImpl });
  },
  local_ydb_pull_image: async (args, options) => {
    const parsed = PullImageArgs.parse(args ?? {});
    return pullImage(
      createContext(
        parsed.profile,
        options.executor,
  ...
  local_ydb_pull_status: async (args) => {
  ...
  local_ydb_write_dynamic_auth_config: async (args, options) => {
    ...
    return writeDynamicNodeAuthConfig(
      createContext(
        parsed.profile,
        options.executor,
        handlerConfig(parsed.configPath, options),
...
function listVersionsSchema(): Tool["inputSchema"] {
  return {
    type: "object",
    properties: {
      image: {
...
function pullImageSchema(): Tool["inputSchema"] {
  return {
    type: "object",
    properties: {
      profile: { type: "string" },
...
function pullStatusSchema(): Tool["inputSchema"] {
  return {
    type: "object",
    required: ["jobId"],
    properties: {
...
function upgradeVersionSchema(): Tool["inputSchema"] {
  return {
    type: "object",
    required: ["version"],
    properties: {
