...
export const localYdbPromptDefinitions = {
  ...
  local_ydb_diagnose_database: {
    ...
    render: (args) => [
      ...
      argumentBlock(args),
      workflowSafety,
      "Run local_ydb_status_report first to capture Docker, tenant, node, auth, and health context. Then run local_ydb_healthcheck with databasePath when supplied. Request noCache=true only when cache-bypass semantics are desired, and noMerge=true only when separate issue entries are desired. After the call, always inspect compatibilityFallback, optionResolution, and warnings: an older YDB CLI may return a health result after dropping an unsupported option, so do not call the result fresh or unmerged unless that option is listed in optionResolution.effective.",
