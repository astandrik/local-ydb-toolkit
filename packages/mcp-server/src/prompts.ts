...
const workflowSafety = [
  ...
  "Call mutating tools without confirm first. After the user explicitly approves the returned exact plan, read its one-time token from confirmation.token, then repeat the same request with confirm=true and that value in the confirmationToken request argument.",
  "Configured auth/password files and a selected standalone restore dump are bound privately to the exact plan. If their contents change, obtain a fresh plan/token; never expose or persist file contents or fingerprints.",
  "Treat confirmationToken as an ephemeral capability: do not log or persist it, and do not treat possession of it as proof of human approval.",
].join("\n");
...
export const localYdbPromptDefinitions = {
  ...
  local_ydb_diagnose_database: {
    ...
    render: (args) => [
      ...
      argumentBlock(args),
      workflowSafety,
      "Run local_ydb_status_report first to capture Docker, tenant, node, auth, and health context. Then run local_ydb_healthcheck with databasePath when supplied. Request noCache=true only when cache-bypass semantics are desired, and noMerge=true only when separate issue entries are desired. After the call, always inspect compatibilityFallback, optionResolution, and warnings: after a rejection matches the exact recognized two-line legacy parser signature, an older YDB CLI may return a health result without the unsupported option, so do not call the result fresh or unmerged unless that option is listed in optionResolution.effective.",
  ...
  local_ydb_auth_hardening_workflow: {
    ...
    render: (args) => [
      ...
      "Call local_ydb_prepare_auth_config without confirm to review the hardened config plan. Call local_ydb_write_dynamic_auth_config without confirm to review the dynamic-node auth token plan, passing sid and tokenHostPath when the selected profile does not provide them. If the user approves both artifact plans, repeat each exact request with confirm=true, copying each plan response's confirmation.token into the confirmationToken request argument to create the files. Then call local_ydb_apply_auth_hardening without confirm to review the restart plan, and repeat that exact request with its token only after explicit approval.",
  ...
  local_ydb_schema_generate_apply_workflow: {
    ...
    render: (args) => [
      ...
      "After generation, review script, scriptSha256, warnings, applyRisk, and validation. Then call local_ydb_apply_schema action=validate with the generated script. For execution, call local_ydb_apply_schema action=apply without confirm first; after the user explicitly approves that exact plan, repeat the same apply request with confirm=true and the plan response's confirmation.token in the confirmationToken request argument.",
