...
export const localYdbPromptDefinitions: readonly LocalYdbPromptDefinition[] = [
  ...
  },
  {
    ...
    render: (args) => [
      ...
      "Schema constraints to preserve: generated scripts have the same 1 MiB limit as apply_schema; use { token: \"ENABLED\" } for bare table WITH tokens; string WITH values remain string literals; use the top-level store field instead of with.STORE; column names must not start with the reserved __ydb_ prefix; partitionByHash only with store: \"column\" and primaryKey columns; column-oriented primary keys must be NOT NULL and use supported key types; secondary indexes and vector indexes are for row-oriented tables; normal secondary indexes are global-only and do not accept creation-time with settings; unique indexes must be synchronous; ALTER TABLE ADD COLUMN supports only name/type, not notNull/default; do not repeat add/drop column names in one alterTable spec; do not add an index on a column added or dropped in the same alterTable spec; vector_kmeans_tree requires non-unique GLOBAL SYNC plus vector_dimension, vector_type, exactly one of distance or similarity, clusters, and levels; prefer adding vector indexes after loading representative data and treat CREATE TABLE vector-index warnings as actionable; destructive DROP TABLE and DROP COLUMN/DROP INDEX warnings are high-risk signals.",
    ].join("\n\n"),
  },
] as const;
