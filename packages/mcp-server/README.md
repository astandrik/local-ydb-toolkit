...
For every mutating tool, call the exact request without `confirm` and review the returned plan. Read the token from that response's `confirmation.token`, then repeat the same request with `confirm: true` and that value in the `confirmationToken` request argument. The process-bound token is one-time and is consumed before the first side effect; a valid submitted token is also retired whenever a mutating confirm call becomes a no-op or plan-only response after refreshed prerequisites, required profile inputs, DDL validation, or SQL `EXPLAIN`. It binds raw execution inputs, including DDL, SQL parameters, secret-bearing command input, and private content fingerprints for configured auth files or the selected standalone restore dump; contents and fingerprints are never returned. Configured auth/password files are limited to 16 MiB before hashing. Internal content placeholders are substituted only in declared command fields, not globally across user arguments. Missing, changed, replayed, or pre-restart tokens execute nothing and return a fresh plan/token. Do not log or persist tokens, and do not treat possession as proof of human approval.

Accepted auth config, password-rotation backup inputs, dynamic-node auth token, and standalone restore payloads execute from private verified snapshots rather than reopening their canonical paths. Composite storage reduction and version upgrade also make a private verified copy of the dump immediately after it is generated and restore only from that copy. Upgrade and storage-reduction confirmations for the same execution host and static container share a target-scoped lease held through mutation and cleanup, regardless of profile name or config source; a second confirm is rejected while the lease is active, and plans issued during that execution become stale when it finishes. Distinct stack targets remain independent. Auth-enabled composite rebuilds reject filesystem aliases (including symlinks and hardlinks) among the three configured auth destinations before rebuilding and again before persistence. They generate config, dynamic token, and root credential in a separate private workspace, persist those bytes to the configured host files, and continue auth reapply and node recreation only from the private copies. Root-password rotation also rechecks both host destinations against the reviewed bytes immediately before backups/writes. A conflict or unreadable/non-regular destination stops persistence without replacing either destination or backup; the runtime password may already have changed, so verify it independently, reconcile the files, and start with a new plan/token. This bounded late check is not atomic against subsequent external writes. Credential files used only to authenticate remain freshness guards. Dynamic tokens are copied into stopped containers before start; restore mounts its snapshot read-only and rejects symlinks or special entries before mutation. Snapshot contents, digests, and temporary paths are not returned and are cleaned after success, failure, or abort. Restore tries copy-on-write or reflink cloning first, but the fallback can temporarily need free space equal to the full dump size. Destroy, restart, and composite teardown bind static and configured dynamic IDs (or explicit absence) as well as one-off IDs. Existing-object actions and static tenant commands use those IDs; rebuilds retain the receipt across dump, check names are absent before shared cleanup, and require vacated names during bootstrap. Dynamic creation owns only the returned create ID for copy/start/error cleanup. Composite teardown uses only the container set and exact Docker IDs captured in its reviewed plan, so a same-name replacement is preserved, and dynamic-node removal is bound to exact inspected Docker IDs. Storage reduction binds the configured image tag to its exact Docker image ID, rechecks it before dump and destructive teardown, enforces it during bootstrap, and verifies final container IDs. Version upgrade never rewrites the profile config; after independent verification, update `profiles.<name>.image` manually.
...
Manual agent smoke check: run the MCP server once with `LOCAL_YDB_MCP_CONTENT_FORMAT=json` and once with `toon`, then call the same tools in both sessions: `local_ydb_inventory`, `local_ydb_status_report`, `local_ydb_healthcheck`, `local_ydb_bootstrap_root_database` without `confirm`, `local_ydb_scheme`, `local_ydb_generate_schema` with `validate=true`, `local_ydb_apply_schema` with `action=validate`, `local_ydb_sql` for a parameterized query, standalone explain, and plan-only execute, `local_ydb_permissions` with a plan-only mutation, `local_ydb_list_versions`, and `local_ydb_nodes_check`. Record whether the agent extracts the same status, planned commands, risks, and next steps. Treat this as qualitative evidence; the benchmark command is the reproducible metric.
...
Mutating tools require `confirm: true` with the plan response's `confirmation.token` supplied in the `confirmationToken` request argument. `local_ydb_apply_schema` is the table-DDL apply path: it validates raw or generated YQL DDL through the official `@ydbjs/*` SDK, supports `PRAGMA`, `CREATE TABLE`, `ALTER TABLE`, and `DROP TABLE`, and applies only after validation succeeds and the repeated request's token is accepted. Responses report the script SHA-256 and capped YDB issue text without echoing the raw script or credential paths. For table creation, prefer a CMS tenant path such as `/local/example`; a root-only `/local` stack can validate DDL through the static endpoint, but YDB rejects storage-backed table creation there when the root database has no tenant storage pools.

The server includes a root-only `local_ydb_bootstrap_root_database` tool for starting `/local` with just the static node, a tenant-oriented `local_ydb_bootstrap` tool for GraphShard-ready dynamic-node topologies, a read-only `local_ydb_list_versions` tool for discovering published `local-ydb` image tags with numeric versions sorted newest first, background image-pull tools (`local_ydb_pull_image` and `local_ydb_pull_status`) for slow registry downloads, `local_ydb_generate_schema` and `local_ydb_apply_schema` for table DDL generation/validation/application, `local_ydb_list_dumps` plus dump/restore tools for tenant-wide or tenant-relative backup operations, and a high-risk `local_ydb_upgrade_version` tool that upgrades a file-backed profile by exact target image-ID preflight, dump, rebuild, restore, auth reapply, and final tag-plus-ID verification. It returns a manual profile-update outcome instead of rewriting the config; unavailable final inventory requires independent verification before that manual update. Bind-mounted data profiles are not supported by the automatic rebuild path.

`local_ydb_dump_tenant` and `local_ydb_restore_tenant` accept `path` while preserving existing tenant-wide defaults. Dump `path` is the source object or directory for `ydb tools dump -p`; restore `path` is the destination directory for `ydb tools restore -p`. Use `local_ydb_list_dumps` before restore to choose a `dumpName`; accepted restore uses a private read-only snapshot, rejects symlinks and special entries, and may temporarily require space equal to the full dump when copy-on-write is unavailable. Use optional `describePaths` plus bounded whole-table `countQueries` such as `SELECT COUNT(*) FROM \`dir/table\`;` when a restore needs post-command verification.
