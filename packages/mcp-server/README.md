...
For every mutating tool, call the exact request without `confirm` and review the returned plan. Read the token from that response's `confirmation.token`, then repeat the same request with `confirm: true` and that value in the `confirmationToken` request argument. The process-bound token is one-time and is consumed before the first side effect. It binds raw execution inputs, including DDL, SQL parameters, secret-bearing command input, and private content fingerprints for configured auth files or the selected standalone restore dump; contents and fingerprints are never returned. Missing, changed, replayed, or pre-restart tokens execute nothing and return a fresh plan/token. Do not log or persist tokens, and do not treat possession as proof of human approval.

Accepted auth config, password-rotation backup inputs, dynamic-node auth token, and standalone restore payloads execute from private verified snapshots rather than reopening their canonical paths. Credential files used only to authenticate remain freshness guards. Dynamic tokens are copied into stopped containers before start; restore mounts its snapshot read-only and rejects symlinks or special entries before mutation. Snapshot contents, digests, and temporary paths are not returned and are cleaned after success, failure, or abort. Restore tries copy-on-write or reflink cloning first, but the fallback can temporarily need free space equal to the full dump size.
