...
- Issue distinct upgrade and storage-reduction tokens for one profile before either confirm; after one is accepted, every other pre-issued rebuild token for that profile must be rejected before a second dump or teardown. A different profile remains independently confirmable.
