...
Arbitrary configuration files remain supported through an absolute `configPath` tool argument or an absolute `LOCAL_YDB_TOOLKIT_CONFIG` value. Explicit files must exist, be readable regular files no larger than 1 MiB, and match the strict configuration schema. An explicit configuration error fails closed instead of selecting the built-in profile. Error responses expose a stable configuration code without returning the file contents, JSON parser context, schema input, stack or cause, or the absolute path.
