...
Both bootstrap paths reuse an existing running or stopped static container only when its exact image reference and current image ID, network, `/ydb_data` mount, complete loopback port bindings, required environment, `unless-stopped` restart policy, and disabled healthcheck match the selected profile. Tenant bootstrap additionally requires GraphShard plus static gRPC and every configured dynamic gRPC binding. Increasing `dynamicNodeCount` therefore rejects a static container created for the smaller topology before any dynamic-node mutation. Any inspect failure or mismatch stops before `docker start` and asks the caller to recreate the container; bootstrap never silently removes or replaces it.
