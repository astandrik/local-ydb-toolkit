...
The plugin version is independent from the MCP npm package version. Plugin `0.1.4` pins `@astandrik/local-ydb-mcp@0.18.0`. Update that pin only in a follow-up change after the exact npm version has been published and read back successfully; do not make release-please point the plugin at an unpublished version.
...
This writes `dist/local-ydb-toolkit-0.1.4-skills.zip`. The generated compatibility manifest omits `mcpServers`, and the ZIP excludes both MCP config files. Submission copy, reviewer cases, and external approval gates are recorded in [`docs/openai-plugin-submission.md`](docs/openai-plugin-submission.md). Building the artifact does not authorize uploading or publishing it.
