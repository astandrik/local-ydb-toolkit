...
release-please uses the default `GITHUB_TOKEN`. The post-release plugin job requires a fine-grained `RELEASE_PLEASE_TOKEN` with repository contents and pull request write access; the workflow exposes it only to the final branch-push/draft-PR step and never falls back to `github.token` for that mutation.
