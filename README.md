...
Start a new Codex session after installation so the bundled skill and MCP server are loaded. The MCP launcher requires Node.js 20.19 or newer plus `npx`; its first start can access the npm registry to install the pinned `@astandrik/local-ydb-mcp@0.17.0` package.
...
The plugin version is independent from the MCP npm package version. Plugin `0.1.3` pins `@astandrik/local-ydb-mcp@0.17.0`. Update that pin only in a follow-up change after the exact npm version has been published and read back successfully; do not make release-please point the plugin at an unpublished version.
