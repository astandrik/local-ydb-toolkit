...
Release Please and final post-release plugin PR creation use the existing fine-grained `RELEASE_PLEASE_TOKEN` with repository contents and pull request write access. Release Please receives it only as the action's token input so its generated pull request can trigger the required checks. Plugin transformation and validation remain tokenless; a separate fresh runner exposes the PAT only to its final idempotency/branch-push/draft-PR step. That step runs no package or checked-out repository scripts, disables Git hooks, and neither credential path falls back to `github.token`.
