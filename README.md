...
- `dynamicNodeCount` configured dynamic tenant nodes, started in index order and verified by exact IC port before the next node starts.
...
`local_ydb_add_dynamic_nodes` adds one-off runtime nodes without requiring separate profile entries. By default it starts at index `dynamicNodeCount + 1`; explicit `startIndex` and port overrides keep their existing meaning. These nodes are not part of the declarative count. The tool starts nodes one at a time and verifies each new IC port through `viewer/json/nodelist` before continuing. Tenant bootstrap always recreates configured dynamic containers from the current profile, including an already-running suffix with stale ports or mounts.

`local_ydb_restart_stack` inventories the stack before planning. It reports configured containers that are missing and suffix containers above `dynamicNodeCount` that are unexpected runtime extras. Restart recreates every configured node from the current profile in index order. Unexpected containers are never removed: previously running ones are stopped and restarted with the same container identity after configured nodes, while previously stopped ones remain stopped. If a base command, configured-node command, readiness check, or unexpected-node recovery fails, restart still attempts every previously-running unexpected container and preserves the original failure before recovery results. Removing a configured suffix explicitly therefore creates runtime drift that the next restart or tenant bootstrap restores.
