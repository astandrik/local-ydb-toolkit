...
release-please uses the default `GITHUB_TOKEN`. The post-release plugin flow requires a fine-grained `RELEASE_PLEASE_TOKEN` with repository contents and pull request write access; tokenless transformation and validation run in earlier jobs, and the workflow exposes the PAT only to the final fresh-runner idempotency/branch-push/draft-PR step. That step runs no package or checked-out repository scripts, disables Git hooks, and never falls back to `github.token`.
