...
`local_ydb_set_root_password` rotates the runtime `root` password with `ALTER USER`, then updates the configured host-side `config.auth.yaml` and `root.password` files to match. Immediately before backups or host writes, both destinations must still match the reviewed bytes and file state (regular files are bounded to 16 MiB). A mismatch or unreadable/non-regular destination stops persistence without replacing either destination or backup. Runtime credentials may already have changed: independently verify the new password, reconcile the host files, and obtain a new plan/token. This late freshness check is not an atomic lock against external writes after the check. The password value is redacted from the planned command text.
